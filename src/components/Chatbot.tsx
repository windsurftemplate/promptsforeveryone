...
const Chatbot = forwardRef<ChatbotRef, ChatbotProps>(({ chatId }, ref) => {
  ...
  return (
    <div className="flex flex-col h-[800px] bg-black rounded-lg border border-[#8B5CF6]/20">
      <div className="p-4 border-b border-[#8B5CF6]/20 flex justify-between items-center">
        <h1 className="text-xl font-bold bg-gradient-to-r from-[#8B5CF6] to-[#8B5CF6] bg-clip-text text-transparent">
          AI Prompt Engineer
        </h1>
        <div className="flex items-center space-x-4">
          <button
            ...
            className="px-4 py-2 text-sm bg-black text-[#8B5CF6] border border-[#8B5CF6] rounded hover:bg-[#8B5CF6]/10 transition-colors"
          >
            New Chat
          </button>
          <button
            ...
            className="px-4 py-2 text-sm bg-black text-[#8B5CF6] border border-[#8B5CF6] rounded hover:bg-[#8B5CF6]/10 transition-colors disabled:opacity-50 disabled:hover:bg-transparent"
          >
            Save as Prompt
          </button>
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 scrollbar-thin scrollbar-thumb-[#8B5CF6]/20 scrollbar-track-transparent">
        {messages.length === 0 && (
          ...
          >
            <div
              className={`max-w-[80%] rounded-lg p-3 ${
                message.role === 'user'
                  ? 'bg-[#8B5CF6]/10 border border-[#8B5CF6]/20 text-white'
                  : 'bg-black border border-[#8B5CF6]/20 text-gray-200'
              }`}
            >
              {message.content}
            </div>
        ...
        {isLoading && (
          <div className="flex justify-start">
            <div className="bg-black border border-[#8B5CF6]/20 text-gray-200 rounded-lg p-3">
              Thinking...
            </div>
          </div>
        )}
      ...
      <form onSubmit={handleSubmit} className="p-4 border-t border-[#8B5CF6]/20">
        <div className="flex space-x-2">
          <input
            ...
            className="flex-1 bg-black text-white rounded-lg px-4 py-2 border border-[#8B5CF6]/20 focus:outline-none focus:border-[#8B5CF6]/50"
            disabled={isLoading}
          />
          <button
            type="submit"
            disabled={isLoading}
            className="px-4 py-2 bg-black text-[#8B5CF6] border border-[#8B5CF6] rounded-lg hover:bg-[#8B5CF6]/10 transition-colors disabled:opacity-50 disabled:hover:bg-transparent"
          >
            Send
          </button>
        </div>
